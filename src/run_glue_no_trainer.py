...
def main():
    ...
    if args.task_name is not None:
        ...
        eval_metric = evaluate.load("glue", args.task_name)
    else:
        train_metric = evaluate.load("accuracy")
        train_metric_f1 = evaluate.load("f1")
    ...
    for epoch in range(args.num_train_epochs):
        ...
        logger.info(f"\ttrain_acc: {train_acc:.5f}\ttrain_f1: {train_f1:.5f}")

        eval_acc = eval_metric.compute()['accuracy']
        eval_f1 = eval_metric_f1.compute(average="macro")['f1']

        logger.info(f"\teval_acc: {eval_acc:.5f}\teval_f1: {eval_f1:.5f}")
        ...
        logger.info(f"best epoch: {best_accuracy_epoch} best eval accuracy: {best_accuracy}")
        logger.info(f"best f1 epoch: {best_f1_epoch} best eval f1: {best_f1}")
